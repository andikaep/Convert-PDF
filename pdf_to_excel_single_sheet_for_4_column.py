...
def extract_columns_from_line(line):
    ...
    if len(parts) < 4:
        ...
        # Gabungkan semua bagian sebelum tiga bagian terakhir sebagai kategori
        category = ' '.join(parts[:-3]).strip()
        amount1 = parts[-3].strip()
        amount2 = parts[-2].strip()
        amount3 = parts[-1].strip()
        return [category, amount1, amount2, amount3]
        ...
        if text:
            ...
            for line in lines:
                if line.strip():  # Abaikan baris kosong
                    ...
                    rows.append(columns)
            rows.append([None, None, None, None])  # Tambahkan baris kosong saat berganti halaman

# Buat DataFrame dari data yang diproses
df = pd.DataFrame(rows, columns=['Category', 'Amount1', 'Amount2', 'Amount3'])
